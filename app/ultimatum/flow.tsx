 ...
 *
 * Stranger-to-stranger, not link-sharing, same change and same reasoning as
 * Trust's flow: the server checks for a real waiting offer (findWaitingRound
 * in lib/pair.ts) before minting a new one, and redirects straight into
 * answering it. Unlike Trust, Ultimatum's payoff math is untouched, its own
 * accept/reject mechanic already prices in a stingy offer, a flat penalty on
 * top would double-punish the same behaviour.
 *
...
export default function Flow({ example, waiting }: { example: WorkedExample; waiting?: Waiting }) {
  ...
  useEffect(() => {
    let dead = false;
    (async () => {
      // A real offer, found by the server (findWaitingRound in lib/pair.ts),
      // beats minting a new one: joining an already-open one is exactly what
      // "stranger-to-stranger, not a link" means. /u/[id] is the existing,
      // correct second-player screen, this just sends someone there
      // automatically instead of making them wait for a link.
      if (waiting) {
        router.push(`/u/${waiting.id}`);
        return;
      }
      ...
      if (cached) {
        try {
          ...
          if (status === "waiting") {
            if (!dead) {
              setRound({ id: cached.id, stake: cached.stake });
      ...
      try {
        ...
        if (status === "revealed") {
          ...
          try {
            ...
            const out = await res.json();
            if (res.ok && !dead) {
              setRound({ id: out.id, stake: out.stake });
              setStage("offer");
            }
  ...
  async function commit() {
    ...
    try {
      ...
      setSentOffer(offer);
      saveOpenRound("ultimatum", {
        id: round.id, link: `${window.location.origin}/u/${round.id}`, at: Date.now(), stake, offer,
      });
      setStage("waiting");
    } catch (e) {
      setErr(readable(e));
  ...
  return (
    <main className="screen">
      <ExperimentHeader experiment="Ultimatum" index={3} />
      <h1>Looking for someone to answer.</h1>
      <p className="soft">
        They&rsquo;ll set their line before they see what you offered, at the
        same time you&rsquo;re guessing it. We&rsquo;ll let you know the
        moment they answer.
      </p>
